...
#   noise_value = random.randint(0,60)
    ...
    video_writer = cv2.VideoWriter(
        path_of_video_to_save, cv2.VideoWriter_fourcc(*fourcc), fps, (w, h))
...
def augment_videos(i):
    try:
        ...
        augment_and_save_frames(
            video_reader, output_folder_path, video_clip_names[clip_no], i, fps, w, h)
        end = time.time()
...
if __name__ == '__main__':
    ...
    for clip_no in range(no_of_clips_available):
        ...
        print("No. of videos to be augmented per input",
              no_of_clips_to_augment_per_frame)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.map(augment_videos, list(
                range(no_of_clips_to_augment_per_frame)))
